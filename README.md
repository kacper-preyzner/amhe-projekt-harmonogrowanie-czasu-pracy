...
Memetyczny algorytm ewolucyjny (własna implementacja NSGA-II + przeszukiwanie lokalne) z
...
(koszt x dopasowanie preferencji), więc wynikiem jest front Pareto kompromisów.
...
./all.sh                      # cały pipeline: setup -> test -> run
./all.sh --smoke              # jak wyżej, ale szybki przebieg dymny
```

To samo przez `make` (`setup` / `test` / `smoke` / `experiments`) lub
bezpośrednio:
...
amhe/
  ...
  analysis/      plots, gantt, stats, tables
...
- Twarde ograniczenia (Kodeks pracy): 8 h/dobę, 40 h/tydzień, odpoczynek dobowy conajmniej 11 h, tygodniowy conajmniej 35 h, praca nocna najwyzej 8 h.
- Stawki 2026: płaca min. 4806 zł/mies., 31,40 zł/h; dodatki nocny (+20%), niedzielny/świąteczny (+100%).
- Kryteria (min.): **koszt** = płace + kara za niedobór obsady; **kara preferencji**  = sloty pracy poza preferowanym oknem pory dnia pracownika.
